...
export default function SaisiePage() {
  ...
  function deleteRow(idx: number) {
    const row = rows[idx];
    if (row.id) supabase.from("ecritures_comptables").delete().eq("id", row.id).then(() => {});
    setRows((prev) => {
    ...
    if (toInsert.length > 0) {
      const { error } = await supabase.from("ecritures_comptables").insert(
        toInsert.map((r) => ({
          ...
          compte_label: r.compte ? getAccountLabel(r.compte) : null,
      ...
      <div className="bg-white border border-[rgba(0,0,0,0.08)] rounded-xl overflow-hidden">
        ...
        <div className="overflow-x-auto">
          <table className="w-full min-w-[720px]">
            ...
            <tbody>
              {loadingRows ? (
                <tr><td colSpan={7} className="py-10 text-center text-[12px] text-[#9CA3AF]">Chargement...</td></tr>
              ) : filteredRows.length === 0 ? (
                <tr><td colSpan={7} className="py-10 text-center text-[12px] text-[#9CA3AF]">Aucune écriture trouvée</td></tr>
