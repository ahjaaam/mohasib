    ...
    <div className="space-y-5">
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {[
          ...
          { label: "Jours déclarés", value: leaves.reduce((s: number, l: EmployeeLeave) => s + Number(l.nombre_jours), 0) },
          { label: "Sans solde", value: leaves.filter((l: EmployeeLeave) => !l.is_paid).length },
          { label: "Jours fériés", value: calendarDays.filter(d => d.isHoliday).length },
        ].map(card => (
          <div key={card.label} className="bg-white border border-[rgba(0,0,0,0.08)] rounded-xl p-3.5">
            ...
            <div className="text-[20px] font-bold text-[#1A1A2E]">{card.value}</div>
          </div>
        ))}
      </div>
              ...
              <div className="min-w-[680px]">
                <div className="grid grid-cols-7 border-b border-[rgba(0,0,0,0.07)] bg-[#F8F8F5]">
                  {weekDays.map(day => (
                    <div
                      key={day}
                      className="px-2 py-2.5 text-center text-[10px] font-bold uppercase tracking-[0.7px] text-[#6B7280]"
                    >
                      {day}
                    </div>
                  ))}
