...
import { MemberToggle } from "@/components/admin/AdminControls";
...
export default async function ResponsablesPage({ searchParams }: { searchParams: Promise<Record<string, string | undefined>> }) {
  ...
  const [memberships, users, companiesRes] = await Promise.all([
    admin.from("user_memberships")
      .select("id,user_id,user_email,first_name,last_name,company_id,role_name,status,invited_at,accepted_at,created_at,companies(raison_sociale)")
      .neq("status", "revoked")
  ...
  return <div>
    <div className="mb-5 flex items-start justify-between gap-3"><div><h1 className="text-xl font-bold">Responsables</h1><p className="mt-1 text-xs text-gray-500">{rows.length} accès Responsable(s), invitations incluses</p></div><ResponsibleInviteButton companies={companies} /></div>
    ...
    <div className="overflow-x-auto rounded-md border border-black/10 bg-white"><table className="w-full text-left text-[11px]"><thead className="bg-[#F8F8F5] text-gray-500"><tr>{["Responsable", "Compte", "Rôle", "Statut", "Dernière connexion", "Depuis", "Actions"].map(value => <th key={value} className="px-3 py-2.5 font-semibold">{value}</th>)}</tr></thead><tbody className="divide-y divide-black/5">{rows.map(member => {
      const company = Array.isArray(member.companies) ? member.companies[0] : member.companies;
      const authUser = member.user_id ? users.get(member.user_id) : null;
      const authBanned = !!authUser?.banned_until && new Date(authUser.banned_until) > new Date();
      return <tr key={member.id}><td className="px-3 py-3"><b>{`${member.first_name ?? ""} ${member.last_name ?? ""}`.trim() || member.user_email || "Invitation"}</b>{member.first_name && <div className="text-gray-400">{member.user_email}</div>}</td><td className="px-3 py-3 font-semibold"><Link href={`/admin/comptes/${member.company_id}`}>{company?.raison_sociale || "—"}</Link></td><td className="px-3 py-3">Responsable</td><td className="px-3 py-3"><StatusBadge status={authBanned ? "banned" : member.status} /></td><td className="px-3 py-3 text-gray-500">{formatDate(authUser?.last_sign_in_at)}</td><td className="px-3 py-3 text-gray-500">{formatDate(member.accepted_at || member.invited_at || member.created_at)}</td><td className="px-3 py-3"><MemberToggle id={member.id} suspended={member.status === "suspended"} authBanned={authBanned} /></td></tr>;
    })}</tbody></table></div>
  </div>;
}
