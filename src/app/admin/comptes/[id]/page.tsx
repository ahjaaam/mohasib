...
export default async function AccountDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <div>
    ...
    <div className="mt-5 grid gap-5 xl:grid-cols-2">
      <section className="rounded-md border border-black/10 bg-white"><h2 className="border-b border-black/10 px-4 py-3 text-sm font-bold">Responsables</h2><div className="divide-y divide-black/5">{(members.data ?? []).map(member => <div key={member.id} className="flex items-center justify-between px-4 py-3 text-[11px]"><div><b>{`${member.first_name ?? ""} ${member.last_name ?? ""}`.trim() || member.user_email || "Invitation"}</b><div className="text-gray-500">{member.user_email} · Responsable · {member.status}</div></div><MemberToggle id={member.id} suspended={member.status === "suspended"} /></div>)}</div></section>
