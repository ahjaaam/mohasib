...
export default async function AccountsPage({ searchParams }: { searchParams: Promise<Record<string, string | undefined>> }) {
  ...
  const [companies, users] = await Promise.all([
    admin.from("companies").select("id, user_id, raison_sociale, email, phone, ice, user_type, plan, subscription_status, subscription_ends_at, trial_ends_at, is_suspended, created_at").order("created_at", { ascending: false }),
    authUserMap(),
  ]);
  ...
  return <div>
    ...
    <div className="overflow-x-auto rounded-md border border-black/10 bg-white"><table className="w-full text-left text-[11px]"><thead className="bg-[#F8F8F5] text-gray-500"><tr>{["Compte", "Email propriétaire", "Téléphone", "Type", "Accès", "Fin", "Créé", "Actions"].map(value => <th key={value} className="px-3 py-2.5 font-semibold">{value}</th>)}</tr></thead><tbody className="divide-y divide-black/5">{rows.map(company => <tr key={company.id} className="hover:bg-black/[.015]"><td className="px-3 py-3 font-bold"><Link href={`/admin/comptes/${company.id}`}>{company.raison_sociale || "Sans nom"}</Link></td><td className="px-3 py-3 text-gray-500">{users.get(company.user_id)?.email || company.email || "—"}</td><td className="px-3 py-3">{company.phone || users.get(company.user_id)?.user_metadata?.phone || "—"}</td><td className="px-3 py-3">{company.user_type || "—"}</td><td className="px-3 py-3"><StatusBadge status={accountStatus(company)} /></td><td className="px-3 py-3">{formatDate(company.subscription_ends_at || company.trial_ends_at)}</td><td className="px-3 py-3 text-gray-500">{formatDate(company.created_at)}</td><td className="px-3 py-3"><AccountActions id={company.id} /></td></tr>)}</tbody></table></div>
  </div>;
}
