...
export default async function AdminDashboard() {
  ...
  return <div>
    ...
    <section className="mt-5 rounded-md border border-black/10 bg-white">
      <div className="flex items-center justify-between border-b border-black/10 px-4 py-3"><div><h2 className="text-sm font-bold">Gestion rapide des comptes</h2><p className="mt-0.5 text-[10px] text-gray-500">Changez un plan ou ouvrez le dossier administratif du compte.</p></div><Link href="/admin/comptes" className="text-[11px] font-semibold text-[#9A672E]">Tous les comptes →</Link></div>
      <div className="overflow-x-auto"><table className="w-full text-left text-[11px]"><thead className="bg-[#F8F8F5] text-gray-500"><tr>{["Société", "Plan", "Statut", "Actions"].map(value => <th key={value} className="px-4 py-2.5 font-semibold">{value}</th>)}</tr></thead><tbody className="divide-y divide-black/5">{rows.slice(0, 8).map(company => <tr key={company.id}><td className="px-4 py-3 font-bold">{company.raison_sociale || "Sans nom"}</td><td className="px-4 py-3">{company.plan || "trial"}</td><td className="px-4 py-3"><StatusBadge status={company.is_suspended ? "suspended" : company.subscription_status} /></td><td className="px-4 py-3"><AccountActions id={company.id} name={company.raison_sociale} currentPlan={company.plan} /></td></tr>)}</tbody></table></div>
    </section>
    <section className="mt-5 rounded-md border border-black/10 bg-white">
