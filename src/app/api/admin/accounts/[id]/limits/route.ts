...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const values = { company_id: id, ocr_limit: numeric("ocr_limit"), storage_gb: numeric("storage_gb"), dossiers_limit: numeric("dossiers_limit"), users_limit: numeric("users_limit"), employee_limit: numeric("employee_limit"), has_paie: flag("has_paie"), has_bank_import: flag("has_bank_import"), has_saisie: flag("has_saisie"), has_export_fiduciaire: flag("has_export_fiduciaire"), has_avoirs: flag("has_avoirs"), has_bilan: flag("has_bilan"), has_tva_edi: flag("has_tva_edi"), has_inbox_global: flag("has_inbox_global"), has_mass_declarations: flag("has_mass_declarations"), has_whatsapp_agent: flag("has_whatsapp_agent"), reason: body.reason, expires_at: body.expires_at || null, created_by_email: user!.email, updated_at: new Date().toISOString() };
  const numericValues = [values.ocr_limit, values.storage_gb, values.dossiers_limit, values.users_limit, values.employee_limit];
  if (numericValues.some(value => value == null || !Number.isFinite(value) || value < -1)) {
    return NextResponse.json({ message: "Chaque limite doit être un nombre supérieur ou égal à −1." }, { status: 400 });
  }
  if ((values.users_limit ?? 0) < 1) {
    return NextResponse.json({ message: "Le compte doit autoriser au moins un utilisateur." }, { status: 400 });
  }
  const { data: old } = await admin!.from("company_limit_overrides").select("*").eq("company_id", id).maybeSingle();
  const { error } = await admin!.from("company_limit_overrides").upsert(values, { onConflict: "company_id" });
  if (error) return NextResponse.json({ message: error.message }, { status: 400 });
  const { data: company } = await admin!.from("companies").select("subscription_status").eq("id", id).maybeSingle();
  if (company?.subscription_status !== "trial") {
    await admin!.from("companies").update({ plan: "custom" }).eq("id", id);
  }
