...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const update = await admin!.from("companies").update({ is_suspended: !!body.suspended, suspended_reason: body.suspended ? body.reason : null }).eq("id", id).select("id,is_suspended").maybeSingle();
  if (update.error) return NextResponse.json({ message: update.error.message }, { status: 400 });
  if (!update.data || update.data.is_suspended !== !!body.suspended) {
    return NextResponse.json({ message: "L’état du compte n’a pas pu être modifié." }, { status: 409 });
  }
