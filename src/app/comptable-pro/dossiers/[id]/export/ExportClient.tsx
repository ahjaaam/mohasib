  ...
  return (
    <div className="max-w-6xl">
      ...
      <section className="mt-4 rounded-xl border border-[rgba(0,0,0,0.08)] bg-white p-5">
        ...
        <div className="mb-4 grid grid-cols-1 gap-2 md:grid-cols-2 xl:grid-cols-3">{DOCUMENTS.map((doc) => { const active = selected.has(doc.id); const Icon = doc.icon; return <button type="button" key={doc.id} onClick={() => toggle(doc.id)} className={`flex min-h-[68px] items-center gap-3 rounded-lg border px-3 py-3 text-left transition-colors ${active ? "border-[rgba(200,146,74,0.45)] bg-[#FFF7ED]" : "border-[rgba(0,0,0,0.08)] bg-white hover:bg-[#FAFAF6]"}`}><span className={`flex h-9 w-9 items-center justify-center rounded-lg ${active ? "bg-[#C8924A] text-white" : "bg-[#F3F4F6] text-[#6B7280]"}`}><Icon size={16} /></span><span className="min-w-0 flex-1"><span className="block text-[11.5px] font-semibold text-[#1A1A2E]">{doc.label}</span><span className="mt-0.5 block text-[10.5px] text-[#9CA3AF]">{doc.format}</span></span>{active && <CheckCircle size={15} className="text-[#C8924A]" />}</button>; })}</div>
