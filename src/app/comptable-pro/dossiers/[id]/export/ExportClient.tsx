...
export default function ExportClient({ dossier }: Props) {
  ...
  async function generatePackage() {
    ...
    try {
      const [invoiceResult, transactionResult, entryResult, receiptResult] = await Promise.all([
        supabase.from("invoices").select("*, clients(name,ice), invoice_payments(montant,date_paiement,mode_paiement,reference,allocation_status)").eq("dossier_id", dossier.id).gte("issue_date", period.start).lte("issue_date", period.end).order("issue_date"),
        ...
        supabase.from("receipts").select("id,file_name,status,created_at,ocr_data,invoice_payments(montant,date_paiement,mode_paiement,reference,allocation_status)").eq("dossier_id", dossier.id).eq("status", "matched"),
      ]);
      ...
      const supplierInvoices = (receiptResult.data ?? []).filter((receipt: any) =>
      ...
      if (selected.has("payment-deadlines-xlsx")) {
        setProgress("Suivi des délais de paiement (Excel)");
        const workbook = XLSX.utils.book_new();
        const metaRows = [
        ...
        const clientSheet = XLSX.utils.aoa_to_sheet([
          ...metaRows,
          [...PAYMENT_DEADLINE_HEADERS],
          ...buildClientPaymentDeadlineRows(invoices, period),
        ]);
        const supplierSheet = XLSX.utils.aoa_to_sheet([
          ...metaRows,
          [...PAYMENT_DEADLINE_HEADERS],
          ...buildSupplierPaymentDeadlineRows(supplierInvoices, period),
        ]);
  ...
  return (
    <div className="max-w-6xl">
      ...
      <section className="mt-4 rounded-xl border border-[rgba(0,0,0,0.08)] bg-white p-5">
        <div className="mb-3 flex flex-wrap items-center justify-between gap-2"><div><div className="text-[13px] font-semibold text-[#1A1A2E]">Documents inclus dans le package</div><div className="mt-0.5 text-[10.5px] text-[#9CA3AF]">Sélectionnez uniquement les fichiers dont vous avez besoin</div></div><div className="flex gap-2"><button onClick={() => setSelected(new Set(DOCUMENTS.map((doc) => doc.id)))} className="btn btn-sm border-[#E7D3B5] bg-[#FAF3E8] text-[#C8924A]">Tout sélectionner</button><button onClick={() => setSelected(new Set())} className="btn btn-sm border-[#D1D5DB] bg-[#F3F4F6] text-[#4B5563]">Effacer</button></div></div>
        ...
        {generating && <div className="mb-4 flex items-center gap-2 rounded-lg bg-[#FAF3E8] px-3 py-2.5 text-[12px] text-[#C8924A]"><RefreshCw size={13} className="animate-spin" /> {progress}</div>}
